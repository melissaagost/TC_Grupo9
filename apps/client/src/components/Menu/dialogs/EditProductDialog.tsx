...
import { Item, ItemDTO } from "../../../types/itemTypes"; // asegurate de tener estos
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const categoriaMatch = categorias.find(
        (cat) => cat.nombre === initialData.categoria.nombre
      );
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 p-3 border border-gray-300 rounded-md">
              <span className="text-gray-500 text-sm">{estado ? "Activo" : "Inactivo"}</span>
              <Switch.Root
                checked={estado}
